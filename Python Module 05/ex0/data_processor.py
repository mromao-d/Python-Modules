...
class InvalideDataException(Exception):
    ...
    def ingest(self, data: int | float | list) -> None:
        ...
        try:
            if not self.validate(data):
                raise InvalideDataException("Got exception: Improper numeric data")
        ...
        except InvalideDataException as e:
            print(e)
...
class LogProcessor(DataProcessor):
    ...
    def output(self) -> tuple[int, str]:
        ...
        for i in range(0, extractions):
            min_val = min(list(self.stored.keys()))
            txt = self.stored[min_val]
            print(f"Numeric Value {min_val}: {txt.get('log_level')}: {txt.get('log_message')}")
            self.stored.pop(min_val)
        return 1, "text"
...
if __name__ == '__main__':
    ...
    for el in ['Hello', {'log_level': 'NOTICE', 'log_message': 'Connection to server'},
               [{'log_level': 'NOTICE', 'log_message': 'Connection to server'}, {'log_level': 'ERROR', 'log_message': 'Unauthorized access!!'}]]:
        print(f"Trying to validate input {el}: {testLog.validate(el)}")
    testLog.ingest([{'log_level': 'NOTICE', 'log_message': 'Connection to server'}, {'log_level': 'ERROR', 'log_message': 'Unauthorized access!!'}])
